    ...
    TriggerEvent("es:getPlayers", function(players)
      ...
      for i,v in pairs(players) do
        ...
        if (identifier ~= nil) then
          local executed_query = MySQL:executeQuery("SELECT identifier, job_id, job_name FROM users LEFT JOIN jobs ON jobs.job_id = users.job WHERE users.identifier = '@identifier' AND job_id = 14 AND enService = 1", {['@identifier'] = identifier[1]})
