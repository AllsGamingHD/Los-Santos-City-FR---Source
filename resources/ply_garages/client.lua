...
AddEventHandler('ply_garages:SpawnVehicle', function(vehicle, plate, state, primarycolor, secondarycolor, pearlescentcolor, wheelcolor)
	...
	Citizen.CreateThread(function()
		...
		exports.pNotify:SendNotification({text = "La zone est encombrée", type = "info", timeout = 10000})
		else
			if state == "Sortit" then
			exports.pNotify:SendNotification({text = "Ce véhicule n'est pas dans le garage", type = "info", timeout = 10000})
			else
				local mods = {}
				for i = 0,24 do
				...
				exports.pNotify:SendNotification({text = "Véhicule sorti", type = "info", timeout = 10000})
...
AddEventHandler('ply_garages:StoreVehicle', function(vehicle, plate)
	...
	Citizen.CreateThread(function()
		...
		exports.pNotify:SendNotification({text = "Ce n'est pas ton véhicule", type = "info", timeout = 10000})
			
			else
				Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(caissei))
				exports.pNotify:SendNotification({text = "Véhicule rentré", type = "info", timeout = 10000})
				TriggerServerEvent('ply_garages:SetVehIn', plate)
				TriggerServerEvent("ply_garages:CheckGarageForVeh")
			end
		else
		exports.pNotify:SendNotification({text = "Aucun véhicule présent", type = "info", timeout = 10000})
