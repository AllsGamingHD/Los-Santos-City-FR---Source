...
TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    local executed_query = MySQL:executeQuery("UPDATE police_outfits SET skin='@skin' WHERE identifier='@user'",{['@skin']=skin,['@user']=player})
    local executed_query2 = MySQL:executeQuery("SELECT identifier FROM police_outfits WHERE identifier='@user'",{['@user']=player})
...
AddEventHandler("skin_customization:ChoosenComponents",function()
    TriggerEvent('es:getPlayerFromId', source, function(user)
	    ...
	    local player = user.identifier
        local executed_query = MySQL:executeQuery("SELECT face,face_text,hair,hair_text,pants,pants_text,shoes,shoes_text,torso,torso_text,shirt,shirt_text FROM police_outfits WHERE identifier='@user'",{['@user']=player})
...
AddEventHandler("skin_customization:SpawnPlayer", function()
	TriggerEvent('es:getPlayerFromId', source, function(user)
        ...
        if(result[1].isFirstConnection == 1)then
            local executed_query3 = MySQL:executeQuery("INSERT INTO police_outfits(identifier) VALUES ('@identifier')",{['@identifier']=player})
            ...
            TriggerClientEvent("skin_customization:Customization", source, "mp_m_freemode_01")
             ...
             local executed_query2 = MySQL:executeQuery("SELECT skin FROM police_outfits WHERE identifier = '@username'", {['@username'] = player})
		    --Récupération des données générée par la requête.
...
AddEventHandler("skin_customization:SaveComponents",function(components)
    TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		local player = user.identifier
        local executed_query = MySQL:executeQuery("UPDATE police_outfits SET @a='@drawid',@b='@textureid' WHERE identifier='@user'",{['@a']=draw,['@drawid']=drawid,['@b']=text,['@textureid']=textid,['@user']=player})
    end)
end)
