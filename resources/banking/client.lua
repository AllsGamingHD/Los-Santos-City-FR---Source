  ...
  Citizen.CreateThread(function()
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
		...
		exports.pNotify:SendNotification({text = "Appuyer sur 'Context Action Key' (Defaut : E) pour l'activer", type = "info", timeout = math.random(1000, 10000)})
        end
        atBank = true
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
		  exports.pNotify:SendNotification({text = "Vous ne pouvez utilisez la banque en étant dans un véhicule!!", type = "info", timeout = 10000})
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
	exports.pNotify:SendNotification({text = "Vous ne pouvez pas utilisez l'ATM dans un véhicule", type = "info", timeout = 10000})
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
  	exports.pNotify:SendNotification({text = "Vous déposez uniquement à la banque", type = "info", timeout = 10000})
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    if (IsInVehicle()) then
		exports.pNotify:SendNotification({text = "Vous ne pouvez pas utilisez la banque dans un véhciule", type = "info", timeout = 10000})
    else
      TriggerServerEvent("bank:withdraw", tonumber(amount))
    end
  else
  	exports.pNotify:SendNotification({text = "Ceci n'est pas une banque ou ATM!", type = "info", timeout = 10000})
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
	  ...
	  exports.pNotify:SendNotification({text = "Ce joueur n'est pas en ligne!", type = "info", timeout = math.random(1000, 10000)})
    end
  else
  exports.pNotify:SendNotification({text = "Vous n'êtes pas à coter du joueurs!", type = "info", timeout = math.random(1000, 10000)})
  end
end)
