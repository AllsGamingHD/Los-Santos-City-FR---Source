...
AddEventHandler('police:getArrested', function()
	if(isCop == false) then
		...
		if(handCuffed) then
			exports.pNotify:SendNotification({text = "Vous êtes menotter", type = "info", timeout = 10000})
		else
			exports.pNotify:SendNotification({text = "Vous n'êtes plus menotter", type = "info", timeout = 10000})
		end
	end
end)
...
AddEventHandler('police:payFines', function(amount)
	...
	exports.pNotify:SendNotification({text = "Vous avez payer une amande de "..amount.."€", type = "info", timeout = 10000})
end)
...
function ServiceOff()
	isInService = false
	TriggerServerEvent("jobssystem:jobs", 12)
