...
function RequestToSave()
	...
	if not origin then
		...
		exports.pNotify:SendNotification({text = "Position Sauvegardée", type = "info", timeout = 1250, layout = "topLeft"})
	end
end
...
AddEventHandler("projectEZ:spawnlaspos", function(PosX, PosY, PosZ)
	...
	if not origin then
			exports.pNotify:SendNotification({text = "Vous voici à votre dernière position", type = "info", timeout = 10000})
    end
	Saver()
end)
